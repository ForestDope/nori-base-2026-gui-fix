...
class flat_sensor(mi.Sensor):
    def __init__(self, props):
        ...
        # Whether the real sensor is sampled at a random sub-pixel
        # position (True) or always at the pixel center (False).
        self.jitter_pixels = True
    ...
    def initialize(
        ...
        self.jitter_pixels = bool(jitter_pixels)
    ...
    def sample_ray_differential(
        ...
        # Compute the sub-pixel offset. When jittering is enabled, take
        # the fractional part of the flat sampler's draw and use it as
        # a random sub-pixel position. When disabled, sample exactly at
        # the pixel centre (0.5, 0.5) so per-iteration gradients are
        # deterministic.
        if self.jitter_pixels:
            ...
            jitter_x = mi.Float(0.5)
            jitter_y = mi.Float(0.5)

        sample2_override = mi.Point2f(
            (target_pixel_x + jitter_x) / float(self.source_film_width),
            (target_pixel_y + jitter_y) / float(self.source_film_height)
        )
    ...
    def traverse(self, callback):
        """Register custom DrJit members so ``dr.freeze`` can discover them."""
        super().traverse(callback)
        if self.pixel_idx is not None:
            callback.put("pixel_idx", self.pixel_idx, mi.ParamFlags.NonDifferentiable)
        callback.put("sensors_unique_ptr", self.sensors_unique_ptr, mi.ParamFlags.NonDifferentiable)
...
class Rayloader():
    def __init__(
        ...
        self.flat_sensor.initialize(sensors, pixels_per_batch, jitter_pixels=jitter_pixels)
